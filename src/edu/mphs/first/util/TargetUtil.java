...
public class TargetUtil {
    ...
    public static TargetUtil[] findCircularTargets(ColorImage image) throws NIVisionException { //include gregs other parameters?
        ...
        SortedVector.Comparator targetComparator = new SortedVector.Comparator() {
            ...
            public int compare(Object target1, Object target2) {
                if (((TargetUtil) target1).m_score > ((TargetUtil) target2).m_score) {
                    return 1;
                }
                if (((TargetUtil) target1).m_score < ((TargetUtil) target2).m_score) {
                    return -1;
        ...
        for (int i = 0; i < results.length; i++) {
            TargetUtil target = new TargetUtil();
        ...
        while (!sortedTargetList.isEmpty()) {
            TargetUtil firstTarget = (TargetUtil) sortedTargetList.elementAt(0);
            for (int i = 1; i < sortedTargetList.size(); i++) {
                TargetUtil otherTarget = (TargetUtil) sortedTargetList.elementAt(i);
        ...
        TargetUtil[] toReturn = new TargetUtil[combinedTargetList.size()];
